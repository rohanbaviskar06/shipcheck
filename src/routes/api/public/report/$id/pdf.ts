...
function wrap(
  ...
  for (const w of words) {
    ...
    try {
      ...
      // Fallback in case of unexpected character
    ...
    handlers: {
      GET: async ({ params }) => {
        ...
        try {
          ...
          if (error || !scan) return new Response("Not found", { status: 404 });
          ...
          y -= 22;
          page.drawText(sanitizeText(scan.url).slice(0, 90), {
            x: left,
            y,
            size: 10,
            font: body,
          ...
          page.drawText(
            sanitizeText(
              `Score ${scan.score}/100 - ${new Date(scan.created_at).toLocaleDateString("en-US", {
                month: "short",
                day: "numeric",
                year: "numeric",
              })}`
